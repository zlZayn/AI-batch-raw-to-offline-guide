...
> **关于多模态：** 国内主流AI编程助手均已支持图片输入。素材若是攻略截图、菜单照片、地图标注等，可直接拖入对话窗口让AI识别。这是本项目的关键能力——有它才能处理非文字素材。
...
3. 上下文窗口够大（装得下整个项目）
4. **能读取图片** — 素材含攻略截图、菜单照片、地图标注、票据时，AI需能识别图片内容并提取数据。操作方式：拖入图片文件，或用 @ 引用。

> **💡 截图素材怎么用：** 小红书/大众点评攻略截图、菜单照片、地图标注、票据等，直接拖给AI即可。AI会先识别图片中的文字和结构，再按Schema填入JSON。截图越清晰、内容越结构化（表格/分点列表），提取准确率越高。
...
2. **素材** — Markdown笔记？截图（攻略、菜单、地图、票据）？备忘录文本？还是你直接口述？
   - 图片素材：请用户拖入对话窗口。你读取图片内容后按Schema结构化。图片本身不进入 data/，需转写为文字再提取。
...
能。主流AI编程助手都支持图片输入。适合小红书/大众点评攻略截图、菜单照片、园区地图标注、票据照片、手写笔记照片等场景。截图越清晰、内容越结构化，提取越准。提取后建议对照原截图检查关键数据（价格、时间、项目名）。
