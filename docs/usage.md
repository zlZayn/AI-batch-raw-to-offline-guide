...
| 类别 | 代表工具 | 说明 |
|------|---------|------|
| **海外** | Cursor、Claude Code、Codex、Windsurf、GitHub Copilot | 功能成熟，图片输入支持完善 |
| **国内** | Trae（字节）、Qoder CN（阿里）、CodeBuddy（腾讯）、Comate（百度） | 中文理解好，均已支持图片/截图输入，免费额度较充足 |

> **关于计费：** 各工具免费额度差异较大，跑一次本项目流程通常够用。需要反复迭代或处理大量素材时，按需选择付费档位即可。
>
> **关于多模态：** 国内主流 AI 编程助手均已支持图片输入。素材若是攻略截图、菜单照片、地图标注等，可直接拖入对话窗口让 AI 识别。这是本项目的关键能力——有它才能处理非文字素材。
...
3. 上下文窗口够大（装得下整个项目）
4. **能读取图片** — 素材含攻略截图、菜单照片、地图标注、票据时，AI 需能识别图片内容并提取数据。操作方式：拖入图片文件，或用 @ 引用。

> **截图素材怎么用：** 小红书/大众点评攻略截图、菜单照片、地图标注、票据等，直接拖给 AI 即可。AI 会先识别图片中的文字和结构，再按 Schema 填入 JSON。截图越清晰、内容越结构化（表格/分点列表），提取准确率越高。
...
```
你说："我想做一个上海迪士尼攻略。素材是十几张小红书截图。"
  → AI 要求你逐张拖入截图
  → AI 识别截图中的项目、排队时间、餐厅推荐
  → 提取为结构化数据，写入 JSON
```
...
> 下面这个代码块是核心。
> **你要做的：从「复制发给 AI — 开始」一直复制到「复制发给 AI — 结束」，完整粘贴给 AI。**
...
2. **素材** — Markdown 笔记？截图（攻略、菜单、地图、票据）？备忘录文本？还是你直接口述？
   - 图片素材：请用户拖入对话窗口。你读取图片内容后按 Schema 结构化。图片本身不进入 data/，需转写为文字再提取。
...
> 以上是完整的 AI 规范。
> **你现在需要做的就是：复制上面从「开始」到「结束」之间的全部内容，粘贴给 AI。**
...
能。主流 AI 编程助手都支持图片输入。适合小红书/大众点评攻略截图、菜单照片、园区地图标注、票据照片、手写笔记照片等场景。截图越清晰、内容越结构化，提取越准。提取后建议对照原截图检查关键数据（价格、时间、项目名）。
